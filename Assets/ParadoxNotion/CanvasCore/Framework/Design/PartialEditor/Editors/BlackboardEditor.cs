    ...
    public class BlackboardEditor : EditorObjectWrapper<IBlackboard>
    {
        ...
        private void OnGraphSelectionChange(IGraphElement element)
        {
            elementDefinedParameterIDs = element != null
                ? Graph.GetParametersInElement(element).Where(p => p != null && p.isDefined)
        ...
        ///<summary>Show variables inspector for target bb. Optionally provide override serialization context object</summary>
        private void InspectorGUI(IBlackboard bb, Object overrideContextObject = null)
        {
            if (bb.parent != null)
            {
        ...
        private object VariableField(Variable data, Object contextParent, GUILayoutOption[] layoutOptions)
        {
            var o = data.value;
            var t = data.varType;
            ...
            if (o == null && t.IsAbstract && !typeof(Object).IsAssignableFrom(t) && !t.IsInterface && t != typeof(Type))
            {
                if (GUILayout.Button("(null) Create", layoutOptions))
                    EditorUtils.GetTypeSelectionMenu(t, derived => { data.value = Activator.CreateInstance(derived); })
            ...
            if (GUILayout.Button(
                    ...
                    layoutOptions)) //we use bb.GetVariableByID to avoid undo creating new instance of variable and thus generic inspector, left inspecting something else
                GenericInspectorWindow.Show(data.name, t, contextParent,
                    () => { return bb.GetVariableByID(data.ID).value; },
                    newValue => { bb.GetVariableByID(data.ID).value = newValue; });
