...
namespace NodeCanvas.Editor
{
    public class TaskEditor : EditorObjectWrapper<Task>
    {
        ...
        {
            TaskFieldMulti(task, ownerSystem, typeof(T), t => { callback((T)t); });
        }
        ...
        {
            ShowCreateTaskSelectionButton(ownerSystem, typeof(T), t => { callback((T)t); });
        }
        ...
        public static void ShowCreateTaskSelectionButton(ITaskSystem ownerSystem, Type baseType, Action<Task> callback)
        {
            ...
            if (GUILayout.Button(label))
            {
                ...
                if (CopyBuffer.TryGetCache(out Task copy) && baseType.IsAssignableFrom(copy.GetType()))
                {
                    menu.AddSeparator("/");
            ...
            GUILayout.BeginHorizontal(GUI.skin.box);
            {
                ...
                if (taskType.IsGenericType)
                {
                            ...
                            GUILayout.Height(20)))
                    {
                        ...
                        menu = EditorUtils.GetPreferedTypesSelectionMenu(taskType.GetGenericTypeDefinition(),
                            t => { callback(Task.Create(t, task.ownerSystem)); }, menu, "Set Generic Type");
                        menu.ShowAsContext();
                    }
                }
        ...
        private GenericMenu GetMenu(Action<Task> callback)
        {
            ...
            menu.AddItem(new GUIContent("Copy"), false, () => { CopyBuffer.SetCache(task); });

            foreach (var _m in taskType.RTGetMethods())
            {
                var m = _m;
                var att = m.RTGetAttribute<ContextMenu>(true);
                if (att != null) menu.AddItem(new GUIContent(att.menuItem), false, () => { m.Invoke(task, null); });
            }

            if (taskType.IsGenericType)
                menu = EditorUtils.GetPreferedTypesSelectionMenu(taskType.GetGenericTypeDefinition(),
                    t => { callback(Task.Create(t, task.ownerSystem)); }, menu, "Change Generic Type");
