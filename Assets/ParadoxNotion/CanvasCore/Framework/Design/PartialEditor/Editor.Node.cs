...
namespace NodeCanvas.Framework
{
    partial class Node
    {
        ...
        ///----------------------------------------------------------------------------------------------
        private static readonly Vector2 MIN_SIZE = new(80, 20);

        ///----------------------------------------------------------------------------------------------
        [SerializeField] [fsIgnoreInBuild] private bool _collapsed;

        [SerializeField] [fsIgnoreInBuild] private Color _color;

        [SerializeField] [fsIgnoreInBuild] private VerboseLevel _verboseLevel = VerboseLevel.Full;
        ...
        public Rect rect
        {
            get => new(_position.x, _position.y, size.x, size.y);
            private set
            {
        ...
        public bool isSelected =>
            GraphEditorUtility.activeElement == this || GraphEditorUtility.activeElements.Contains(this);
        ...
        private static void HandleEvents(Node node, Event e)
        {
            ...
            {
                if (node.nodeIsPressed) node.TrySortConnectionsByPositionX();
        ...
        private static GenericMenu GetNodeMenu_Multi(Graph graph)
        {
            ...
            menu.AddItem(new GUIContent("Copy Selected Nodes"), false,
                () =>
                {
                    CopyBuffer.SetCache(Graph.CloneNodes(GraphEditorUtility.activeElements.OfType<Node>().ToList())
                        .ToArray());
                });
        ...
        public static GenericMenu GetNodeMenu_Single(Node node)
        {
            ...
            menu.AddItem(new GUIContent("Copy Node"), false, () => { CopyBuffer.SetCache(new[] { node }); });
            ...
            if (node is ITaskAssignable)
            {
                ...
                if (CopyBuffer.TryGetCache(out Task copy))
                    menu.AddItem(new GUIContent("Paste Assigned Task"), false, () =>
                    {
                        ...
                        {
                            Logger.LogWarning("Can't paste Task here. Incombatible Types", LogTag.EDITOR, node);
                        }
                    });
                else
            ...
            if (menu != null)
            {
                ...
                foreach (var _m in node.GetType().RTGetMethods())
                {
                    ...
                    if (att != null) menu.AddItem(new GUIContent(att.menuItem), false, () => { m.Invoke(node, null); });
                }
