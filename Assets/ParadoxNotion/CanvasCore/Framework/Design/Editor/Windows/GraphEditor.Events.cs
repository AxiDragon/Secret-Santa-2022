...
namespace NodeCanvas.Editor
{
    ...
    partial class GraphEditor
    {
            ...
            if (GraphEditorUtility.allowClick)
            {
                ...
                if (isContext || isShortcut)
                {
                    GenericMenuBrowser.ShowAsync(e.mousePosition, "Add Node", graph.baseNodeType, () =>
                    {
                        ...
                        if (CopyBuffer.TryGetCache(out Node[] copiedNodes) && copiedNodes.Length > 0)
                            if (copiedNodes[0].GetType().IsSubclassOf(graph.baseNodeType))
                            {
                                menu.AddSeparator("/");
                                var suffix = copiedNodes.Length == 1
                                    ? copiedNodes[0].GetType().FriendlyName()
        ...
        {
            Action<Type> Selected = type => { GraphEditorUtility.activeElement = graph.AddNode(type, canvasMousePos); };
