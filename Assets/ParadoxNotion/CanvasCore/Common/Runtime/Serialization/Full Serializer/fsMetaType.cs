...
namespace ParadoxNotion.Serialization.FullSerializer
{
    ...
    {
        private static Dictionary<Type, fsMetaType> _metaTypes = new();
        ...
        private static ObjectGenerator GetGenerator(Type reflectedType)
        {
            ...
            if (typeof(ScriptableObject).IsAssignableFrom(reflectedType))
                return () => { return ScriptableObject.CreateInstance(reflectedType); };

            if (reflectedType
                .IsArray) // we have to start with a size zero array otherwise it will have invalid data inside of it
                return () => { return Array.CreateInstance(reflectedType.GetElementType(), 0); };

            if (reflectedType == typeof(string)) return () => { return string.Empty; };

            if (reflectedType.IsValueType || reflectedType.RTIsDefined<fsUninitialized>(true) ||
                !HasDefaultConstructor(reflectedType))
                return () => { return FormatterServices.GetSafeUninitializedObject(reflectedType); };
