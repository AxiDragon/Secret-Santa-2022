...
namespace ParadoxNotion.Design
{
    ...
    public class GenericMenuBrowser : PopupWindowContent
    {
        ...
        private readonly Color hoverColor = new(0.5f, 0.5f, 1, 0.3f);
        ...
        private readonly Type currentKeyType;
        ...
        public static GenericMenuBrowser Show(GenericMenu newMenu, Vector2 pos, string title, Type keyType)
        {
        ...
        public override void OnGUI(Rect rect)
        {
            ...
            if ((treeGenerationThread != null && treeGenerationThread.ThreadState != ThreadState.Stopped) ||
        ...
        private void DoTree(Rect treeRect, Event e)
        {
            ...
            if (search != lastSearch)
            {
                ...
                if (!string.IsNullOrEmpty(
                        search)) //provide null reference thread (thus no aborting) so that results update all the time
            ...
            foreach (var childPair in currentNode.children)
            {
                ...
                if (isSearch)
                {
                    ...
                    if (memberInfo == null || memberInfo is Type)
                        searchCategory = node.parent.fullPath != null
                            ? node.parent.fullPath.Split(new[] { '/' }, StringSplitOptions.RemoveEmptyEntries)
                                .FirstOrDefault()
                            : null;
                    else
        ...
        private void DoFooter(Rect helpRect, Event e)
        {
            ...
            var hoveringNode = hoveringIndex >= 0 && currentNode.children.Count > 0
                ? currentNode.children.Values.ToList()[hoveringIndex]
                : null;
        ...
        {
            public readonly Dictionary<string, List<string>> allFavorites;
