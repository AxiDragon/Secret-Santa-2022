...
namespace ParadoxNotion.Design
{
    ...
    partial class EditorUtils
    {
        ...
        private static readonly Dictionary<Type, bool> registeredEditorFoldouts = new();
        ...
        {
            return LayerMaskField(string.IsNullOrEmpty(prefix) ? GUIContent.none : new GUIContent(prefix), layerMask,
                layoutOptions);
        }
        ...
        public static IList ListEditor(GUIContent content, IList list, Type listType, InspectedFieldInfo info)
        {
            ...
            list = ReorderableList(list, options,
                (i, r) => { list[i] = ReflectedFieldInspector("Element " + i, list[i], argType, info); });
            ...
            InspectedFieldInfo info)
        {
            ...
            if (GUILayout.Button("Add Element"))
            {
                if (!typeof(UnityObject).IsAssignableFrom(keyType))
                {
                    ...
                    {
                        Logger.LogWarning(string.Format("Key '{0}' already exists in Dictionary", newKey), "Editor");
                        return dict;
                    }
        ...
        {
            return Popup(GUIContent.none, selected, options, GUIOptions);
        }

        ///<summary>Generic Popup for selection of any element within a list</summary>
        public static T Popup<T>(string prefix, T selected, IEnumerable<T> options, params GUILayoutOption[] GUIOptions)
        {
            return Popup(string.IsNullOrEmpty(prefix) ? GUIContent.none : new GUIContent(prefix), selected, options,
                GUIOptions);
        }
        ...
        {
            ButtonPopup(string.IsNullOrEmpty(prefix) ? GUIContent.none : new GUIContent(prefix), selected, options,
                Callback);
        }
            ...
            if (GUILayout.Button(buttonText, "MiniPopup", GUILayout.Width(0), GUILayout.ExpandWidth(true)))
            {
                ...
                foreach (var _option in options)
                {
                    ...
                    menu.AddItem(new GUIContent(option != null ? option.ToString() : "[NONE]"),
                        Equals(selected, option), () => { Callback(option); });
                }
