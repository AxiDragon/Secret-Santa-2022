...
namespace NodeCanvas.Tasks.Conditions
{
    ...
    {
        [RequiredField] public BBParameter<GameObject> checkTarget;

        [SliderField(1, 180)] public BBParameter<float> viewAngle = 70f;

        protected override string info => checkTarget + " in view angle";

        protected override bool OnCheck()
        {
            return Vector2.Angle((Vector2)checkTarget.value.transform.position - (Vector2)agent.position, agent.right) <
                   viewAngle.value;
        }
