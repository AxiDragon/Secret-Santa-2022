...
namespace FMODUnity
{
    public class EventReferenceUpdater : EditorWindow
    {
        ...
        [NonSerialized] private static readonly GUIContent AssetContent = new("Asset");

        private static readonly GUIContent ComponentTypeContent = new("Component Type");
        private static readonly GUIContent GameObjectContent = new("Game Object");
        ...
        private void OnGUI()
        {
            ...
            {
                if (GUILayout.Button("Cancel", GUILayout.Height(buttonHeight))) Cancel();

                using (new EditorGUI.DisabledScope(IsProcessing))
                {
                    if (GUILayout.Button(SearchButtonText, GUILayout.Height(buttonHeight))) BeginSearching();
...
#pragma warning restore 0618
        ...
        private static IEnumerable<Task> GetGenericUpdateTasks(Object target)
        {
            var fields = target.GetType().GetFields(DefaultBindingFlags);

            var oldFields = fields.Where(IsEventRef).ToList();

            var initialOldFieldCount = oldFields.Count;

            var newFields = fields.Where(f => f.FieldType == typeof(EventReference)).ToList();
...
#pragma warning restore 0618
            ...
            foreach (var newField in newFields)
            {
                ...
                var updateTask = GetUpdateEventReferenceTask(eventReference, newField.Name);
                if (updateTask != null) yield return updateTask;
            }
        }
...
#pragma warning disable 0618
                ...
                Implement(Type.GenericRemoveEventRefField,
                    data => { return string.Format("Remove field <b>{0}</b>", data[1]); },
                    ...
                    IsValid: (data, target) =>
                    {
                        var behaviourType = target.GetType();
                        var field = behaviourType.GetField(data[1]);
                ...
                Implement(Type.GenericRemoveEmptyEventRefField,
                    data => { return string.Format("Remove empty field <b>{0}</b>", data[0]); },
                    ManualInstructions: (data, component) =>
                    {
                        return string.Format(
                    ...
                    {
                        var behaviourType = target.GetType();
                        var field = behaviourType.GetField(data[0]);
                ...
                Implement(Type.GenericMoveEventRefFieldToEventReferenceField,
                    ...
                    {
                        var value = data[0];
                        var oldFieldName = data[1];
                        var newFieldName = data[2];

                        var behaviourType = target.GetType();

                        var oldField = behaviourType.GetField(oldFieldName, DefaultBindingFlags);
                        var newField = behaviourType.GetField(newFieldName, DefaultBindingFlags);
                ...
                Implement(Type.GenericUpdateEventReferencePath,
                    ...
                    (data, target) =>
                    {
                        ...
                        var field = targetType.GetField(data[0], DefaultBindingFlags);
...
#pragma warning restore 0618
                ...
                string value, string oldFieldName, string newFieldName)
            {
                return new Task
                {
                    ...
                    Data = new[] { value, oldFieldName, newFieldName }
                };
            }
            ...
            public static Task AddMigrationTarget(string value, string fieldName, string targetName = null)
            {
                return new Task
                {
                    ...
                    Data = new[] { value, fieldName, targetName }
                };
            }
                ...
                GUID guid)
            {
                return new Task
                {
                    ...
                    Data = new[] { fieldName, oldPath, newPath, guid.ToString() }
                };
            }

            public static Task UpdateEventReferenceGuid(string fieldName, GUID oldGuid, GUID newGuid,
                string path)
            {
                return new Task
                {
                    type = Type.GenericUpdateEventReferenceGuid,
                    Data = new[] { fieldName, oldGuid.ToString(), newGuid.ToString(), path }
                };
            }
        ...
        private class Icons
        {
            ...
            public static Texture2D GetAssetIcon(AssetType type)
            {
                ...
                if (type == AssetType.Prefab)
                    return Prefab;
                if (type == AssetType.PrefabModel)
                    return PrefabModel;
                if (type == AssetType.PrefabVariant)
                    return PrefabVariant;
                if (type == AssetType.ScriptableObject)
                ...
                switch ((Column)columnIndex)
                {
                    case Column.Asset:
                        if (Event.current.type == EventType.Repaint)
                        {
                            ...
                            using (new GUI.GroupScope(rect))
                            {
                                ...
                                if (hasGameObjectPath) type.text += " on";
                    ...
                    case Column.Task:
                        if (Event.current.type == EventType.Repaint)
                        {
                            ...
                            if (task.IsManual()) text = "Manual task: " + text;
