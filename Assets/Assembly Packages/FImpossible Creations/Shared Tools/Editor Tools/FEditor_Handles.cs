...
public static class FEditor_TransformHandles
{
    ...
    public static Vector3 DrawAndSetPositionForHandle(Vector3 position, Transform rootReference)
    {
        ...
        var rotation = Tools.pivotRotation != PivotRotation.Local ? Quaternion.identity : rootReference.rotation;

        var size = HandleUtility.GetHandleSize(position) * 0.125f;
        Handles.SphereHandleCap(0, position, rotation, size, EventType.Repaint);
        var pos = Handles.PositionHandle(position, rotation);
    ...
    public static bool DrawSphereHandle(Vector3 position, string text = "")
    {
        ...
        if (Event.current.button != 1)
        {
            ...
            var size = HandleUtility.GetHandleSize(position) * 0.2f;
            ...
            var labelSize = new Vector2(EditorGUIUtility.singleLineHeight * 2, EditorGUIUtility.singleLineHeight);
            var labelPos = HandleUtility.WorldToGUIPoint(position);
    ...
    public static Quaternion RotationHandle(Quaternion rotation, Vector3 position, float size = 1f,
    ...
    public static Vector3 ScaleHandle(Vector3 scale, Vector3 position, Quaternion rotation, float size,
        bool scaleAll = false, bool worldScale = false)
    {
        var handleSize = size;
        if (worldScale) handleSize = HandleUtility.GetHandleSize(position) * size;
        ...
        Handles.color = Handles.centerColor;
        EditorGUI.BeginChangeCheck();
        var num1 = Handles.ScaleValueHandle(scale.x, position, rotation, handleSize, Handles.CubeHandleCap, 0.001f);

        if (EditorGUI.EndChangeCheck())
        {
            var num2 = num1 / scale.x;
            scale.x = num1;
            scale.y *= num2;
            scale.z *= num2;
        }
    ...
    public static Vector3 PositionHandle(Vector3 position, Quaternion rotation, float size = 1f,
        bool worldScale = false, bool freeHandle = true, bool colorize = true)
    {
        var handleSize = size;
        if (worldScale) handleSize = HandleUtility.GetHandleSize(position) * size;
        ...
        {
            Handles.color = Handles.centerColor;
            position = Handles.FreeMoveHandle(position, rotation, handleSize * 0.15f, Vector3.one * 0.001f,
