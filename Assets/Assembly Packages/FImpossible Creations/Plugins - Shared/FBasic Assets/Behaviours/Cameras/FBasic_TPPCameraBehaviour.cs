...
namespace FIMSpace.Basics
{
    ...
    public class FBasic_TPPCameraBehaviour : MonoBehaviour
    {
        ...
        public Vector2 RotationRanges = new(-5f, 60f);

        [Space(10)] [Tooltip("Sensitivity value for rotating camera around following object")]
        public float RotationSensitivity = 10f;

        [Header("If you want camera rotation to be smooth")] [Range(0.1f, 1f)]
        // V1.1
        public float RotationSpeed = 1f;

        [Header("If you want camera to follow target with some smoothness")] [Range(0f, 1f)]
        // V1.1
        ...
        private void FollowCalculations()
        {
            ...
            if (RotationSpeed < 1f)
                animatedSphericRotation = new Vector2(
                    Mathf.LerpAngle(animatedSphericRotation.x, targetSphericRotation.x,
                        Time.deltaTime * 30 * RotationSpeed),
                    Mathf.LerpAngle(animatedSphericRotation.y, targetSphericRotation.y,
                        Time.deltaTime * 30 * RotationSpeed));
            else animatedSphericRotation = targetSphericRotation;

            var rotation = Quaternion.Euler(animatedSphericRotation.y, animatedSphericRotation.x, 0f);
            transform.rotation = rotation;

            var targetPosition = ToFollow.transform.position + FollowingOffset;

            if (HardFollowValue < 1f)
            {
                var lerpValue = Mathf.Lerp(0.5f, 40f, HardFollowValue);
        ...
        {
            var followPoint = ToFollow.transform.position + FollowingOffset +
                              transform.TransformVector(FollowingOffsetDirection);
            var cameraDir = Quaternion.Euler(targetSphericRotation.y, targetSphericRotation.x, 0f);
            var directionRay = new Ray(followPoint, cameraDir * -Vector3.forward);

            // If there is something in sight ray way
            if (Physics.Raycast(directionRay, out sightObstacleHit, targetDistance + CollisionOffset, SightLayerMask,
            ...
            {
                var rotationOffset = transform.rotation * -Vector3.forward * animatedDistance;
                transform.position = targetPosition + rotationOffset +
                                     transform.TransformVector(FollowingOffsetDirection);
            }
        }
        ...
        private void SwitchCalculations()
        {
            if (LockCursor)
                ...
                {
                    HelperSwitchCursor();
                    if (Cursor.visible) rotateCamera = false;
