...
{
    /// <summary>
    ///     FM: Class which contains many helpful methods which operates on Vectors and Quaternions or some other floating
    ///     point maths
    /// </summary>
    public static class FEngineering
    {
        ...
        {
            if (vec.sqrMagnitude == 0f) return true;
            return false;
        ...
        public static Vector3 TransformVector(this Quaternion parentRot, Vector3 parentLossyScale,
        ...
        public static Vector3 TransformInDirection(this Quaternion childRotation, Vector3 parentLossyScale,
            Vector3 childLocalPos)
        {
            return childRotation * Vector3.Scale(childLocalPos,
                new Vector3(parentLossyScale.x > 0 ? 1 : -1, parentLossyScale.y > 0 ? 1 : -1,
                    parentLossyScale.y > 0 ? 1 : -1));
        }
        ...
        public static Quaternion ClampRotation(this Vector3 current, Vector3 bounds)
        {
            ...
            else if (current.x > bounds.x) current.x = bounds.x;
            if (current.y < -bounds.y) current.y = -bounds.y;
            else if (current.y > bounds.y) current.y = bounds.y;
            if (current.z < -bounds.z) current.z = -bounds.z;
            else if (current.z > bounds.z) current.z = bounds.z;
        ...
        public static Vector3 QToAngularVelocity(this Quaternion currentRotation, Quaternion targetRotation,
            bool fix = false)
        {
        ...
        {
            if (rot.x != 0f) return false;
            if (rot.y != 0f) return false;
            if (rot.z != 0f) return false;
            return true;
        }
        ...
        public static Quaternion SmoothDampRotation(this Quaternion current, Quaternion target,
            ref Quaternion velocityRef, float duration, float delta)
        {
            return SmoothDampRotation(current, target, ref velocityRef, duration, Mathf.Infinity, delta);
        }

        public static Quaternion SmoothDampRotation(this Quaternion current, Quaternion target,
            ref Quaternion velocityRef, float duration, float maxSpeed, float delta)
        {
            ...
            var correction = Vector4.Project(new Vector4(velocityRef.x, velocityRef.y, velocityRef.z, velocityRef.w),
                smoothVal);
        ...
        {
            var sum = 0f;
            var functionV = 1f / baseV;
            var i = index;
            while (i > 0)
            {
                sum += functionV * (i % baseV);
        ...
        public static Bounds RotateBoundsByMatrix(this Bounds b, Quaternion rotation)
        {
            ...
            var newB = new Bounds();
            var fr1 = rot.MultiplyPoint(new Vector3(b.max.x, b.min.y, b.max.z));
            var br1 = rot.MultiplyPoint(new Vector3(b.max.x, b.min.y, b.min.z));
            var bl1 = rot.MultiplyPoint(new Vector3(b.min.x, b.min.y, b.min.z));
            var fl1 = rot.MultiplyPoint(new Vector3(b.min.x, b.min.y, b.max.z));
            newB.Encapsulate(fr1);
            newB.Encapsulate(br1);
            newB.Encapsulate(bl1);
            newB.Encapsulate(fl1);

            var fr = rot.MultiplyPoint(new Vector3(b.max.x, b.max.y, b.max.z));
            var br = rot.MultiplyPoint(new Vector3(b.max.x, b.max.y, b.min.z));
            var bl = rot.MultiplyPoint(new Vector3(b.min.x, b.max.y, b.min.z));
            var fl = rot.MultiplyPoint(new Vector3(b.min.x, b.max.y, b.max.z));
            newB.Encapsulate(fr);
            newB.Encapsulate(br);
            newB.Encapsulate(bl);
            newB.Encapsulate(fl);
