...
namespace FIMSpace
{
    ...
    public static class FVectorMethods
    {
        ...
        {
            return new Vector3(Random.Range(rangeA, rangeB), Random.Range(rangeA, rangeB),
        ...
        public static Vector3 RandomVectorMinMax(float min, float max)
        {
            ...
            return new Vector3(Random.Range(min, max) * mul1, Random.Range(min, max) * mul2,
                Random.Range(min, max) * mul3);
        }
        ...
        /// <summary>
        ///     Returning position on screen for UI element in reference to position in world 3D space, the 'z' will be negative if
        ///     text is behind camera
        /// </summary>
        public static Vector3 GetUIPositionFromWorldPosition(Vector3 position, Camera camera, RectTransform canvas)
        {
            var uiPosition = camera.WorldToViewportPoint(position);
        ...
        {
            var abs = new Vector3(Mathf.Abs(axis.x), Mathf.Abs(axis.y), Mathf.Abs(axis.z));
        ...
        {
            var f = forward == null ? Vector3.forward : forward.Value;
