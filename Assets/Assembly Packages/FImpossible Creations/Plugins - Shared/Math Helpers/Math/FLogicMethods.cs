...
namespace FIMSpace
{
    ...
    public static class FLogicMethods
    {
        ...
        {
            var commaVal = 1 / Mathf.Pow(10, afterComma) + addRange;

            if ((val > to - commaVal && val < to + commaVal) || val == to) return true;
        ...
        public static float FLerp(float a, float b, float t, float factor = 0.01f)
        {
            ...
            var val = Mathf.LerpUnclamped(a, b, t);
            if (preB > a)
            {
