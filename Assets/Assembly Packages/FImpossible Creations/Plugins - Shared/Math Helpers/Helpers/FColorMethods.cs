...
namespace FIMSpace
{
    ...
    public static class FColorMethods
    {
        ...
        {
            return new Color(color.r + brightenOrDarken, color.g + brightenOrDarken, color.b + brightenOrDarken,
                color.a);
        }
        ...
        /// </summary>
        public static void LerpMaterialColor(this Material mat, string property, Color targetColor,
            float deltaMultiplier = 8f)
        {
            if (mat == null) return;

            if (!mat.HasProperty(property))
            {
                Debug.LogError("Material " + mat.name + " don't have property '" + property + "' " + " in shader " +
                               mat.shader.name);
                return;
            }

            var currentColor = mat.GetColor(property);
