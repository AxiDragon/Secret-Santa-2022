...
namespace FIMSpace.BonesStimulation
{
    public partial class BonesStimulator
    {
        ...
        [Space(4)] [Range(0f, 1f)] public float MusclesRapidity = .6f;

        [Range(0.05f, 1f)] public float Damping = .4f;

        [Space(4)] [Range(0f, 1f)] public float Smoothing = .6f;

        [Range(0f, 1f)] public float MildRotation;

        [Space(2)] [Tooltip("If moving in world space should have lower influence on the muscles")] [Range(0f, 1f)]
        ...
        {
            var newValue = Mathf.SmoothDamp(StimulatorAmount, targetAmount, ref sd_amount, duration, Mathf.Infinity,
                Time.deltaTime);
