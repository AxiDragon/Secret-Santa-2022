...
namespace FIMSpace.BonesStimulation
{
    public partial class BonesStimulator
    {
        ...
        public void UpdateVibrateLogics()
        {
            ...
            var intensityLog = Mathf.Max(0f, Mathf.Log(_vib_Intensity) / 5f);
            ...
            if (!DetailedVibrate)
            {
                ...
                _vib_sinuses[1] = Mathf.Sin(_vib_Time / 2.2f * _vib_RandomFloats[4] + _vib_RandomFloats[1]) *
                                  _vib_Range;
                ...
                _vib_cosinuses[2] = Mathf.Cos(_vib_Time * 1.24f * _vib_RandomFloats[3] + _vib_RandomFloats[0]) *
                                    _vib_Range;
            }
        ...
        {
            var newIntensity = Mathf.Min(_vib_Intensity + value, value * 3f);
            _vib_Time += value / 3f;
        ...
        protected void Vibrate_ChooseNewSeed()
        {
            ...
            for (var i = 0; i < 3; i++) _vib_RandomFloats[i] = Random.Range(0.8f, 1.0f);
            for (var i = 3; i < 6; i++) _vib_RandomFloats[i] = Random.Range(1.0f, 2.5f);
        ...
        {
            var vibrationFactor = Vector3.zero;

            if (DetailedVibrate)
            {
                _vib_sinuses[0] = Mathf.Sin(_vib_Time * target.randomFloats[0] + target.randomFloats[1]) * _vib_Range;
                _vib_sinuses[1] = Mathf.Sin(_vib_Time / 2.2f * target.randomFloats[4] + target.randomFloats[1]) *
                                  _vib_Range;
                ...
                _vib_cosinuses[2] = Mathf.Cos(_vib_Time * 1.24f * target.randomFloats[3] + target.randomFloats[0]) *
                                    _vib_Range;
            }
            ...
            {
                var posBlend = _vib_blend * VibratePosBlend.Evaluate(target.Evaluation);
                if (posBlend > 0f)
                    target.transform.localPosition += vibrationFactor * 0.005f * VibratePosition * posBlend;
            }

            if (VibrateRotation != 0f)
            {
                var rotBlend = _vib_blend * VibrateRotBlend.Evaluate(target.Evaluation);
                if (rotBlend > 0f)
                {
                    if (UniRotation == false)
                        target.transform.localRotation *=
                            Quaternion.Euler(vibrationFactor * VibrateRotation * rotBlend);
                    else
                        target.transform.rotation = Quaternion.Euler(vibrationFactor * VibrateRotation * rotBlend) *
                                                    target.transform.parent.rotation * target.transform.localRotation;
                }
            }

            if (VibrateScale != 0f)
            {
                var scaleBlend = _vib_blend * VibrateScaleBlend.Evaluate(target.Evaluation);
                if (scaleBlend > 0f)
                    target.transform.localScale += vibrationFactor * 0.003f * VibrateScale * scaleBlend;
            }
        }

        private float Pow2(float toPower)
        {
            return toPower * toPower;
        }
