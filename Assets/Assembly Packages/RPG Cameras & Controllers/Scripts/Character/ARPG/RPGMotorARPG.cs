...
namespace JohnStairs.RCC.Character.ARPG
{
    ...
    {
        /// <summary>
        ///     If set to true and while standing, the character begins moving forward after it completely turned into the new
        ///     direction
        /// </summary>
        ...
        protected override void Awake()
        {
            ...
            if (!_rpgCamera)
                Debug.LogWarning("No RPGCamera component attached to game object " + name +
                                 ". Please assign the script to use this motor.");
        }
        ...
        public override void StartMotor()
        {
            ...
            if (!_canMove)
            {
                ...
                if (EnableSliding && !_swimming) // Still apply sliding
                    ApplySliding();
            }
            else if (_grounded || _enable3dMovement)
            {
                ...
                _runningJump = false;
                var inMotionAlready = IsInMotion();

                if (_autorunning) _inputDirection.z = 1.0f;

                _movementDirection = CalculateMovementDirection();

                if (_movementDirection != Vector3.zero) _facingDirection = _movementDirection;

                // Calculate the movement speed
                var resultingSpeed = ApplyMovementSpeedMultipliers(RunSpeed);
                ...
                {
                    if (_swimming) PreventSwimmingAboveSurface();
                    ...
                    if (_surface)
                    {
                        ...
                        if (_animator && IsCloseToWaterSurface()) _animator.SetTrigger("Jump");
                    }
                    else if (_dive)
                    {
                        _movementDirection.y = -resultingSpeed;
                ...
                else
                {
                    ...
                    if (_jump) // The character is not swimming and should jump this frame
                        PerformJump();
                }

                if (EnableSliding && !_swimming) // Apply sliding
                    ApplySliding();
            }
            else
            {
            ...
            if (!IsLookingInDirection(_facingDirection))
            {
                ...
                var upReference = _enable3dMovement && (_surface || _dive)
                    ? Vector3.Cross(_facingDirection, transform.right)
                    : Vector3.up;

                var targetRotation = Quaternion.LookRotation(_facingDirection, upReference);
                // Smoothly rotate to the target rotation
                var delta = Quaternion.Angle(transform.rotation, targetRotation);
                var t = Mathf.SmoothDampAngle(delta, 0, ref _rotationVelocity, RotationTime);
                t = 1.0f - t / delta;
                transform.rotation = Quaternion.Slerp(transform.rotation, targetRotation, t);

                _turningDirectionY = transform.rotation.eulerAngles.y - _turningDirectionY;

                if (_turningDirectionY < -180.0f) _turningDirectionY += 360.0f;
            }

            if (_canRotate) // Rotate the character along the local X axis
                if (_enable3dMovement)
                {
        ...
        public override void SetAnimatorParameters(Animator animator)
        {
            ...
            var inputMagnitude = IsInMotion() ? _inputDirection.magnitude : 0;
            var inputMagnitudeSmoothing = _crouching || _swimming ? 0.1f : 0;
        ...
        protected virtual Vector3 CalculateMovementDirection()
        {
            ...
            {
                var forward = (_characterInfo.GetTargetPosition() - transform.position).normalized;
            ...
            {
                var right = _rpgCamera.CameraToUse.transform.right;
                //Vector3 forward = Vector3.Cross(right, _enable3dMovement ? _rpgCamera.CameraToUse.transform.up : Vector3.up);
                var forward = Vector3.Cross(right,
                    _swimming || _flyingBuffer < 0 ? _rpgCamera.CameraToUse.transform.up : Vector3.up);

                movementDirection = right * _inputDirection.x + forward * _inputDirection.z;
            }

            // Normalize the player's movement direction
            if (movementDirection.magnitude > 1) movementDirection = Vector3.Normalize(movementDirection);
        ...
        public override void Rotate(Quaternion rotation)
        {
            ...
            if (!IsCombatLockActive() && !IsInMotion()) _facingDirection = transform.forward;
        }
