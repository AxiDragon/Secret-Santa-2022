...
namespace JohnStairs.RCC.Character.ARPG
{
    ...
    public class RPGCameraARPG : RPGCamera
    {
        protected override void LateUpdate()
        {
            ...
            var enableCameraLookUp = !(_rpgMotor?.IsSwimming() ?? false)
            ...
            {
                var cameraToTarget = _characterInfo.GetTargetPosition() - CameraToUse.transform.position;
                cameraToTarget.y = 0;
                var cameraToCharacter = transform.position - CameraToUse.transform.position;
                cameraToCharacter.y = 0;
                if (!Utils.IsAlmostEqual(cameraToTarget, cameraToCharacter))
                {
                    var delta = Utils.SignedAngle(cameraToCharacter.normalized, cameraToTarget.normalized, Vector3.up);
            ...
            if (_distanceSmooth > 0.1f) // In third person => orbit camera
                CameraToUse.transform.LookAt(_cameraPivotPosition);
            else // In first person => normal camera rotation
        ...
        protected override Vector3 ComputeCameraPosition(float xAxisDegrees, float yAxisDegrees, float distance)
        {
            ...
            var rotXaxis = Quaternion.AngleAxis(xAxisDegrees, Vector3.right);
        ...
        public override void MoveTo(Vector3 newPosition, bool smoothTransition = false)
        {
            ...
            var rotationX = Utils.SignedAngle(-Vector3.forward, lookDirection, Vector3.up);
