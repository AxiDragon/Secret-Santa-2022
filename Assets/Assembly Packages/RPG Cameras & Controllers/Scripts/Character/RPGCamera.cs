...
namespace JohnStairs.RCC.Character
{
    public abstract class RPGCamera : MonoBehaviour
    {
        ...
        /// <summary>
        ///     Enum value for setting up which camera object should be used by the script, e.g. the main camera of the scene, a
        ///     newly spawned camera object or the camera assigned to "CameraToUse"
        /// </summary>
        public CameraUsage UsedCamera = CameraUsage.MainCamera;

        /// <summary>
        ///     Reference to the camera object used by the script. If no camera is assigned to this variable, a new one will be
        ///     generated automatically when entering the play mode
        /// </summary>
        public Camera CameraToUse;

        /// <summary>
        ///     Skybox which is currently used or should be used by the camera object. The skybox can be changed at runtime by
        ///     calling the script's method "SetUsedSkybox(material)". Direct assignments to this variable have no effect
        /// </summary>
        ...
        /// </summary>
        public Vector3 CameraPivotLocalPosition = new(0, 0.5f, 0);

        /// <summary>
        ///     Enables the intelligent pivot that moves away from obstacles which the player could see through if zooming in
        ///     enough (internal pivot only)
        /// </summary>
        ...
        /// <summary>
        ///     If set to false, all camera controls are disabled. Can be used to turn off the camera when interacting with a GUI
        ///     (e.g. see the demo GUI interaction)
        /// </summary>
        ...
        /// </summary>
        /// <returns></returns>
        public Color UnderwaterFogColor = new(0, 0.13f, 0.59f);
        ...
        /// <summary>
        ///     For fine-tune the threshold where the camera is considered as underwater. The higher the value, the earlier the
        ///     underwater effects kick in
        /// </summary>
        ...
        /// </summary>
        protected float _desiredRotationY;
        ...
        /// </summary>
        protected float _distanceSmooth;
        ...
        /// <summary>
        ///     Stored on start: If true, the pivot is internal, i.e. inside the character collider. Controls the activation of
        ///     needed functionality for each case
        /// </summary>
        protected bool _internalPivot;

        /// <summary>
        ///     For pyramid view frustums, multiple occlusion checks are needed in succession to account for the changing shape
        ///     (closer distance => less peak stretching)
        /// </summary>
        ...
        /// </summary>
        protected float _rotationXSmooth;
        ...
        /// </summary>
        protected float _rotationYSmooth;
        ...
        protected virtual void Start()
        {
            ...
            {
                Debug.LogWarning(
                    "Main Camera should be used but could not be found in the scene! Spawning a camera...");
                UsedCamera = CameraUsage.SpawnOwnCamera;
            }
            else if (UsedCamera == CameraUsage.AssignedCamera && !CameraToUse)
            {
                Debug.LogWarning(
                    "Assigned Camera should be used but variable \"Camera To Use\" has not been assigned! Spawning a camera...");
                UsedCamera = CameraUsage.SpawnOwnCamera;
            }
            ...
            if (UsedCamera == CameraUsage.SpawnOwnCamera)
            {
                ...
                var camObject = new GameObject(transform.name + transform.GetInstanceID() + " Camera");
            ...
            if (UsedSkybox) // Set the used camera's skybox to the user prescribed one
            ...
            if (!UseNewInputSystem && CursorBehaviorOrbiting == CursorBehavior.Stay)
                Debug.LogWarning(
                    "Cursor Behavior While Orbiting is set to \"Stay\" but is only supported with Unity's new input system! Choose another behavior or set variable \"UseNewInputSystem\" to true");
        ...
        {
            var cyan = new Color(0.0f, 1.0f, 1.0f, 0.65f);
            ...
            else
            {
                ...
                var cameraStartPosition = ComputeCameraPosition(StartRotationY, GetStartRotationX(), StartDistance);
        ...
        protected virtual void OnTriggerEnter(Collider other)
        {
            ...
            if (water) // Store the water script for getting the right water level later
        ...
        {
            var water = other.GetComponent<Water>();
            if (water) // Remove the water again since we left it
                _touchedWaters.Remove(water);
        }

        /// <summary>
        ///     Tries to get the inputs used by this script. If logWarnings is true, warnings are logged if inputs of Unity's
        ///     legacy input system are not set up
        /// </summary>
        /// <param name="logWarnings">
        ///     If true and Unity's legacy input system is used, warnings are logged whenever an input could
        ///     not be found
        /// </param>
        protected virtual void GetInputs(bool logWarnings = false)
        {
            if (UseNewInputSystem)
            {
                ...
                _inputAllowOrbitingWithCharRotation =
                    _inputActions.Character.AllowOrbitingwithCharacterRotation.ReadValue<float>() > 0;
            ...
            else
            {
                ...
                // Orbiting with character rotation input
                _inputAllowOrbitingWithCharRotation =
                    Utils.TryGetButton(Utils.InputPhase.Pressed, "Fire2", logWarnings);
                _inputAllowOrbitingWithCharRotationStart =
                    Utils.TryGetButton(Utils.InputPhase.Down, "Fire2", logWarnings);

                // Orbit rotation input
                var inputRotationAmountX = Utils.TryGetAxis(Utils.InputPhase.Smoothed, "Mouse X", logWarnings);
                var inputRotationAmountY = Utils.TryGetAxis(Utils.InputPhase.Smoothed, "Mouse Y", logWarnings);
                ...
                _inputMaxDistanceZoom =
                    Utils.TryGetButton(Utils.InputPhase.Pressed, "Maximum Distance Zoom", logWarnings);
            }
        }
        ...
        protected virtual void SetUpInputActionCallbacks()
        {
            ...
            _inputActions.Character.AllowOrbitingwithCharacterRotation.canceled +=
                context => _inputAllowOrbitingWithCharRotationStop = true;
        ...
        /// <summary>
        ///     Handles the cursor behavior and visibility depending on the variables "CursorBehaviorWhileOrbiting" and
        ///     "HideCursor"
        /// </summary>
        protected virtual void HandleCursor()
        {
                ...
                || _orbitingStartedOverGUI)
            {
                ...
                if (CursorBehaviorOrbiting == CursorBehavior.LockInCenter) // Unlock the cursor again
                    Cursor.lockState = CursorLockMode.None;

                if (HideCursor == CursorHiding.WhenOrbiting) // Show the cursor again
                    Cursor.visible = true;
                return;
            }

            var orbitingStart = _inputAllowOrbitingStart || _inputAllowOrbitingWithCharRotationStart;
            var orbiting = _inputAllowOrbiting || _inputAllowOrbitingWithCharRotation;
            var orbitingStop = _inputAllowOrbitingStop || _inputAllowOrbitingWithCharRotationStop;
            ...
            {
                if (orbitingStart) // Lock the cursor
                    Cursor.lockState = CursorLockMode.Locked;
                else if (orbitingStop) // Unlock the cursor again
                    Cursor.lockState = CursorLockMode.None;
            }
            ...
            {
                if (orbiting) // Hide the cursor
                    Cursor.visible = false;
                else if (orbitingStop) // Show the cursor again
                    Cursor.visible = true;
            }
            else
            {
        ...
        {
            if (_pointerInfo?.IsPointerOverGUI() ?? false) // Do not zoom if the cursor is over the GUI
                return;
            ...
            else if (_inputMaxDistanceZoom) _desiredDistance = MaxDistance;
        }
        ...
        /// <summary>
        ///     Computes the new camera position based on the desired position. Checks for occlusions between the pivot and the
        ///     camera
        ...
        {
            var closestPivotDistance = Mathf.Infinity;
            // Check if the shape of the view frustum
            var pyramidViewFrustum = _rpgViewFrustum.Shape == RPGViewFrustum.FrustumShape.Pyramid;
            ...
            if (_internalPivot)
            {
                ...
                if (EnableIntelligentPivot)
                {
                    ...
                    var halfHeight = CameraToUse.nearClipPlane *
                                     Mathf.Tan(CameraToUse.fieldOfView * 0.5f * Mathf.Deg2Rad);
                    var halfWidth = halfHeight * CameraToUse.aspect;
                    var halfDiagonal = Mathf.Sqrt(halfWidth * halfWidth + halfHeight * halfHeight);

                    var horizontal =
                        Mathf.Sqrt(halfWidth * halfWidth + CameraToUse.nearClipPlane * CameraToUse.nearClipPlane);
                    var vertical = Mathf.Sqrt(halfHeight * halfHeight +
                                              CameraToUse.nearClipPlane * CameraToUse.nearClipPlane);
                    var diagonal = Mathf.Sqrt(halfDiagonal * halfDiagonal +
                                              CameraToUse.nearClipPlane * CameraToUse.nearClipPlane);

                    Vector3[] rayDirections =
                    {
                        Vector3.forward * horizontal, -Vector3.forward * horizontal, Vector3.left * horizontal,
                        Vector3.right * horizontal, Vector3.up * vertical, Vector3.down * vertical,
                    ...
                    foreach (var ray in rayDirections) // Cast the ray
                        //Debug.DrawRay(_desiredPivotPosition, ray, Color.magenta);
                        if (Physics.Raycast(_desiredPivotPosition, ray, out var hit, ray.magnitude,
                                _rpgViewFrustum.OccludingLayers, QueryTriggerInteraction.Ignore))
                        {
                            // Process the hit
                            var hitDirection = hit.point - _desiredPivotPosition;
                            moveDirection += hitDirection.normalized * (hit.distance - ray.magnitude);
                        }

                    // Smooth the resulting evasive movement 
                    _pivotMoveDirection = Vector3.SmoothDamp(_pivotMoveDirection, moveDirection,
                        ref _pivotCurrentVelocity, PivotSmoothTime);
            ...
            else
            {
                ...
                var colliderHead = GetColliderHeadPosition();

                _pivotRetreat.y = Mathf.Min(_desiredPivotPosition.y, colliderHead.y);

                var extraLength = Vector3.zero;
                if (pyramidViewFrustum) // Add extra length to have the full distance checked (no near clip plane subtracted)
                ...
                closestPivotDistance =
                    _rpgViewFrustum.CheckForOcclusion(_pivotRetreat, _desiredPivotPosition + extraLength, CameraToUse);
            ...
            if (closestPivotDistance == Mathf.Infinity)
            {
                ...
                if (!_internalPivot ||
                    !EnableIntelligentPivot) // External pivot or intelligent pivot disabled. Otherwise, the pivot position computed by the intelligent pivot above would be overwritten
                ...
                var closestDistance =
                    _rpgViewFrustum.CheckForOcclusion(_cameraPivotPosition, _desiredPosition, CameraToUse);

                if (closestDistance == Mathf.Infinity)
                {
                    // Camera view at the desired position is not contrained => smooth the distance change
                    _distanceSmooth = Mathf.SmoothDamp(_distanceSmooth, _desiredDistance, ref _distanceCurrentVelocity,
                        DistanceSmoothTime);
                ...
                else
                {
                    ...
                    var dir = (_desiredPosition - _cameraPivotPosition).normalized;
                    ...
                    if (pyramidViewFrustum)
                    {
                        ...
                        do
                        {
                            ...
                            distance = _rpgViewFrustum.CheckForOcclusion(_cameraPivotPosition, closestPosition,
                                CameraToUse);
                            _maxCheckIterations++;
                        ...
                        // Camera view is constrained => set the camera distance to the closest possible distance 
                        if (_distanceSmooth <
                            closestDistance) // Smooth the distance if we move from a smaller constrained distance to a bigger constrained distance
                            _distanceSmooth = Mathf.SmoothDamp(_distanceSmooth, closestDistance,
                                ref _distanceCurrentVelocity, DistanceSmoothTime);
                        else // Do not smooth if the new closest distance is smaller than the current distance
            ...
            else
            {
                ...
                _cameraPivotPosition = _pivotRetreat +
                                       (_desiredPivotPosition - _pivotRetreat).normalized * closestPivotDistance;
            ...
            _rpgViewFrustum.DrawFrustum(_pivotRetreat, _desiredPivotPosition, CameraToUse,
                closestPivotDistance != Mathf.Infinity);
        ...
        public virtual bool HasInternalPivot()
        {
            ...
            if (!_collider)
            {
                ...
                // Check again
                if (!_collider) // No collider component found at all => assume an external pivot
                    return false;
            }
        ...
        {
            return CameraToUse.transform.position.y <
                   (_touchedWaters.Max?.GetHeight() ?? -Mathf.Infinity) + UnderwaterThresholdTuning;
        }
        ...
        /// <summary>
        ///     Gets the position of the collider head in world space. The collider head is expected to be a safe retreat for the
        ///     external camera pivot
        /// </summary>
        ...
        protected virtual Vector3 GetColliderHeadPosition()
        {
            ...
            {
                var collider = (CharacterController)_collider;
                return transform.TransformPoint(collider.center) +
                       Vector3.up * collider.height * (0.5f - collider.radius);
            }

            if (_collider is CapsuleCollider)
            {
                var collider = (CapsuleCollider)_collider;
                return transform.TransformPoint(collider.center) +
                       Vector3.up * collider.height * (0.5f - collider.radius);
            }
            ...
            {
                var collider = (SphereCollider)_collider;
                return transform.TransformPoint(collider.center);
            }
        ...
        /// <summary>
        ///     Sets the internal variable values that define the camera position. If smoothTransition is passed as true, the
        ///     position change will be immediate
        /// </summary>
        ...
        ///     If true, the transition to the newly set internal variables will be smoothed over time.
        ///     Otherwise, the camera is teleported immediately
        /// </param>
        ...
        /// <param name="smoothTransition">
        ///     If true, the transition to the new position will be smoothed over time. Otherwise, the
        ///     camera is teleported immediately
        /// </param>
        public abstract void MoveTo(Vector3 newPosition, bool smoothTransition = false);

        /// <summary>
        ///     Resets the camera view behind the character + starting X rotation, starting Y rotation and starting distance
        ///     StartDistance
        /// </summary>
        /// <param name="smoothTransition">
        ///     If true, the transition to the default variable values will be smoothed over time.
        ///     Otherwise, the camera is teleported immediately
        /// </param>
        ...
        /// </summary>
        protected bool _inputAllowOrbiting;

        /// <summary>
        ///     If true, pressing _inputAllowOrbiting started this frame
        /// </summary>
        protected bool _inputAllowOrbitingStart;

        /// <summary>
        ///     If true, pressing _inputAllowOrbiting stopped this frame
        /// </summary>
        protected bool _inputAllowOrbitingStop;

        /// <summary>
        ///     Same as above but additionally enables character rotation while orbiting
        /// </summary>
        protected bool _inputAllowOrbitingWithCharRotation;

        /// <summary>
        ///     If true, pressing _inputAllowOrbitingWithCharRotation started this frame
        /// </summary>
        protected bool _inputAllowOrbitingWithCharRotationStart;

        /// <summary>
        ///     If true, pressing _inputAllowOrbitingWithCharRotation stopped this frame
        /// </summary>
        protected bool _inputAllowOrbitingWithCharRotationStop;

        /// <summary>
        ///     Rotation amount along the camera X axis, i.e. around the character's Y axis, as long as orbiting is
        ///     allowed/triggered
        /// </summary>
        ...
        /// </summary>
        protected float _inputZoomAmount;

        /// <summary>
        ///     Fast zoom into first person view
        /// </summary>
        protected bool _inputMinDistanceZoom;

        /// <summary>
        ///     Fast zoom out to max character distance
        /// </summary>
        protected bool _inputMaxDistanceZoom;
