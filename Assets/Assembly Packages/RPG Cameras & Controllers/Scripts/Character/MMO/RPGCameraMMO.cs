...
namespace JohnStairs.RCC.Character.MMO
{
    ...
    public class RPGCameraMMO : RPGCamera
    {
        ...
        /// <summary>
        ///     Determines if the camera should also rotate when the character starts to rotate. If set to "PreventOnInput", input
        ///     "Pause Rotation With Character" is used for pausing rotation
        /// </summary>
        public RotationWithCharacter RotateWithCharacter = RotationWithCharacter.PreventOnInput;

        /// <summary>
        ///     Determines when the character's view/walking direction should be set to the camera's view direction. If set to
        ///     "OnAlignmentInput", input "Align Character" is used to trigger the alignment
        /// </summary>
        ...
        /// <summary>
        ///     If set to true, the camera view direction aligns with the character's view/walking direction once the character
        ///     starts to move (forward/backwards/strafe)
        /// </summary>
        ...
        /// </summary>
        protected bool _characterYrotationAligned;
        ...
        /// </summary>
        protected bool _turningRoutineStarted;
        ...
        protected override void LateUpdate()
        {
            ...
            if (AlwaysRotateCamera && AlignCameraWhenMoving)
            {
                ...
                Debug.LogWarning(
                    "Automatically disabled variable AlignCameraWhenMoving to prevent interference with AlwaysRotateCamera logic");
            }

            // Check if the camera's Y rotation is contrained by terrain
            var enableCameraLookUp = !(_rpgMotorMMO?.IsSwimming() ?? false)
                                     && _rpgViewFrustum.IsTouchingGround(CameraToUse, _cameraPivotPosition);

            var smoothTime = RotationSmoothTime;
            var rotationYMinLimit = _rotationY;
            var combatLock = _characterInfo?.LockOnTarget() ?? false;
            var enable3dMovement = _rpgMotor?.Is3dMovementEnabled() ?? false;
            var isSwimming = _rpgMotor?.IsSwimming() ?? false;
            var stoppedSwimming = _swamLastFrame && !isSwimming;
            var isFlying = _rpgMotor?.IsFlying() ?? false;
            var stoppedFlying = _flewLastFrame && !isFlying;
            ...
            {
                var alsoRotateCharacter = (_characterInfo?.CanRotate() ?? true)
                                          && (_inputAllowOrbitingWithCharRotation
                                              || AlignCharacter == CharacterAlignment.Always
                                              || (AlignCharacter == CharacterAlignment.OnAlignmentInput &&
                                                  _inputAlignCharacter));
                ...
                if (!LockRotationX)
                {
                    ...
                    if (alsoRotateCharacter
                        && !(_pointerInfo?.IsPointerOverGUI() ??
                             false)) // Let the character rotate according to the rotation X axis input
                        _rpgMotorMMO?.RotateAroundAxis(Axis.Y, rotationXinput, RotationXSensitivity);
                    else // Allow the camera to orbit
                        _rotationX += rotationXinput * RotationXSensitivity;

                    if (ConstrainRotationX) // Clamp the rotation in X axis direction
                ...
                if (!LockRotationY)
                {
                    ...
                    if (alsoRotateCharacter
                        ...
                        _rpgMotorMMO.RotateAroundAxis(Axis.X, (InvertRotationY ? -1 : 1) * _inputRotationAmount.y,
                            RotationYSensitivity);
                }

                // Check if a look-up should be performed because _rotationY is constrained by terrain
                if (enableCameraLookUp)
                {
                    _rotationY = Mathf.Clamp(_desiredRotationY, Mathf.Max(rotationYMinLimit, RotationYMin),
                        RotationYMax);
            ...
            if (_rpgMotorMMO
                ...
                && !combatLock)
            {
                var startDiving = _rpgMotorMMO.StartsDiving()
                                  || _rpgMotorMMO.StartedAutorunning();

                var stopDiving = _rpgMotorMMO.DiveOnlyWhenSwimmingForward &&
                ...
                if (AlignCharacter == CharacterAlignment.Always)
                {
                    ...
                    if (enable3dMovement)
                    {
                        ...
                        else if (stopDiving) AlignCharacterWithCamera(TurningRotation.ResetXaxis);
                    }
                }
                         ...
                         && !(_pointerInfo?.IsPointerOverGUI() ?? false))
                {
                    ...
                    if (enable3dMovement)
                    {
                        if (_inputAlignCharacter &&
                            startDiving) // Align both axis rotation of the character to the camera to start diving
                            AlignCharacterWithCamera(TurningRotation.BothAxes);
                        else if (stopDiving) // Reset the X axis rotation every time the character stops diving
                            AlignCharacterWithCamera(TurningRotation.ResetXaxis);
                        else if (_inputAlignCharacterStart &&
                                 _rpgMotorMMO
                                     .HasVerticalMovementInput()) // Alignment input started while the character moves forward => dive in the new direction
                            AlignCharacterWithCamera(TurningRotation.BothAxes);
                        else if (_inputAlignCharacterStart) AlignCharacterWithCamera(TurningRotation.OnlyYaxis);
                    }
                    else
                    {
                ...
                else if (AlignCharacter == CharacterAlignment.Never)
                {
                    // Never align the character with the camera
                    if (enable3dMovement &&
                        stopDiving) // Only take care that the character X axis rotation is reset after diving
                        AlignCharacterWithCamera(TurningRotation.ResetXaxis);
                }

                if (stoppedSwimming || stoppedFlying)
                {
                    if (_turningCoroutine != null) StopCoroutine(_turningCoroutine);
            ...
            var deltaX = _rpgMotorMMO?.GetYRotationDegrees() ?? 0;
            if (deltaX != 0
                &&
                !_inputAllowOrbitingWithCharRotation) // The character turns and does not strafe => check the RotateWithCharacter value
                if (RotateWithCharacter == RotationWithCharacter.Never
                    || (RotateWithCharacter == RotationWithCharacter.PreventOnInput &&
                        _inputPauseRotationWithCharacter))
                {
            ...
            if (ActivateCameraControl) ComputeDesiredDistance();
                ...
                && !_turningRoutineStarted)
            {
                ...
                var inputDirection = _rpgMotorMMO.GetInputDirection();
            ...
            _rotationXSmooth =
                Mathf.SmoothDamp(_rotationXSmooth, _rotationX, ref _rotationXCurrentVelocity, smoothTime);
            _rotationYSmooth =
                Mathf.SmoothDamp(_rotationYSmooth, _rotationY, ref _rotationYCurrentVelocity, smoothTime);
            ...
            else
            {
                ...
                var characterRotation = Quaternion.Euler(new Vector3(0, transform.eulerAngles.y, 0));
                var cameraRotation = Quaternion.Euler(new Vector3(_rotationYSmooth, _rotationXSmooth, 0));
            ...
            if (enableCameraLookUp /*|| _distanceSmooth <= 0.1f*/)
            {
                ...
                var lookUpDegrees = _desiredRotationY - _rotationY;
        ...
        protected override void GetInputs(bool logWarnings = false)
        {
            ...
            if (UseNewInputSystem)
            {
                ...
                _inputPauseRotationWithCharacter =
                    _inputActions.Character.PauseRotationWithCharacter.ReadValue<float>() > 0;
            }
            else
            {
                ...
                _inputPauseRotationWithCharacter = Utils.TryGetButton(Utils.InputPhase.Pressed,
                    "Pause Rotation With Character", logWarnings);
            }
        }
        ...
        protected override Vector3 ComputeCameraPosition(float xAxisDegrees, float yAxisDegrees, float distance)
        {
            ...
            var charXaxisMappedToGroundLayer = transform.right;
            ...
            var rotXaxis = Quaternion.AngleAxis(xAxisDegrees, charXaxisMappedToGroundLayer);
            var rotYaxis = Quaternion.AngleAxis(yAxisDegrees, Vector3.up);
            var rotation = rotYaxis * rotXaxis;
        ...
        {
            var rotYaxis = Quaternion.AngleAxis(yAxisDegrees + transform.eulerAngles.y, Vector3.up);
        ...
        {
            var lookDirection = _cameraPivotPosition - newPosition;
            var distance = lookDirection.magnitude;

            var rotationX = Utils.SignedAngle(transform.forward, lookDirection, Vector3.up);
            var rotationY = Vector3.Angle(Vector3.up, lookDirection) - 90.0f;
        ...
        /// </summary>
        /// <param name="behindCharacter">
        ///     If true, the camera aligns behind the character, otherwise it aligns so that it faces the
        ///     character's front
        /// </param>
        protected virtual void AlignCameraWithCharacter(bool behindCharacter)
        {
            var offsetToCameraRotation = Utils.CustomModulo(_rotationX, 360.0f);
            var targetRotation = behindCharacter ? 0 : 180.0f;

            if (offsetToCameraRotation ==
                targetRotation) // There is no offset to the camera rotation => no alignment computation required
                return;

            var numberOfFullRotations = (int)_rotationX / 360;
        ...
        {
            if (_turningRoutineStarted) StopCoroutine(_turningCoroutine);
        ...
        protected virtual IEnumerator TurningCoroutine(TurningRotation rotationAxes)
        {
            ...
            var targetRotation = GetCharacterTargetRotation(rotationAxes);
            ...
            while (Quaternion.Angle(transform.rotation, Quaternion.Euler(targetRotation)) > 2.0f)
            {
                ...
                transform.rotation = Quaternion.RotateTowards(transform.rotation, Quaternion.Euler(targetRotation),
                    AlignCharacterSpeed * 100.0f * Time.deltaTime);

                var deltaY = transform.rotation.eulerAngles.y - before.eulerAngles.y;
        ...
        /// <summary>
        ///     Get the target rotation euler angles for a turning routine depending on the turning rotation which should be
        ///     performed
        /// </summary>
        ...
        /// </summary>
        protected bool _inputAlignCharacter;

        /// <summary>
        ///     If true, pressing _inputAlignCharacter started this frame
        /// </summary>
        protected bool _inputAlignCharacterStart;

        /// <summary>
        ///     If true, pressing _inputAlignCharacter stopped this frame
        /// </summary>
        protected bool _inputAlignCharacterStop;

        /// <summary>
        ///     Input for preventing camera rotation in cases the camera implicitely rotates with the character, e.g. because the
        ///     character rotates
        /// </summary>
        protected bool _inputPauseRotationWithCharacter;
