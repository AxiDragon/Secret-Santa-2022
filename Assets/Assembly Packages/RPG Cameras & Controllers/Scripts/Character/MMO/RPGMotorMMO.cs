...
namespace JohnStairs.RCC.Character.MMO
{
    ...
    public class RPGMotorMMO : RPGMotor
    {
        ...
        /// </summary>
        public bool UnlimitedMidairMoves;
        ...
        /// </summary>
        protected float _yRotationDegrees;
        ...
        public override void StartMotor()
        {
            ...
            else if (_grounded || _enable3dMovement)
            {
                ...
                if (_movementDirection.magnitude > 1) _movementDirection = Vector3.Normalize(_movementDirection);

                #region Calculate the movement speed

                var resultingSpeed = RunSpeed;
                // Compute the speed combined of strafe and run speed
                if (_inputDirection.x != 0 || _inputDirection.z != 0)
                    resultingSpeed = ((StrafeSpeed + (_characterInfo?.GetMovementSpeedInfluence(StrafeSpeed) ?? 0)) *
                                      Mathf.Abs(_inputDirection.x)
                                      + (RunSpeed + (_characterInfo?.GetMovementSpeedInfluence(RunSpeed) ?? 0)) *
                                      Mathf.Abs(_inputDirection.z))
                ...
                if (_inputDirection.z < 0 && !_walking) resultingSpeed *= BackwardsSpeedMultiplier;
            ...
            else
            {
                ...
                if (_allowMidairMovement)
                {
                    ...
                    {
                        var inputDirectionWorld = transform.TransformDirection(_inputDirection);
        ...
        {
            if (EnableMidairMovement == MidairMovement.Never) return;
        ...
        public virtual void RotateAroundAxis(Axis rotationAxis, float input, float speed = -1)
        {
            ...
            if (speed < 0) speed = RotationSpeed * Time.deltaTime;

            if (rotationAxis == Axis.X)
                _xRotation += input * speed;
            else if (rotationAxis == Axis.Y) _yRotation += input * speed;
        }
        ...
        public override void TeleportTo(Transform target, bool withYrotation = false)
        {
            ...
            if (withYrotation) transform.rotation = Quaternion.Euler(new Vector3(0, target.rotation.eulerAngles.y, 0));
