...
namespace JohnStairs.RCC.Character
{
    public class RPGViewFrustum : MonoBehaviour
    {
        ...
        /// <summary>
        ///     Only objects in these layers are processed by the view frustum. They either cause a camera zoom or are faded out if
        ///     a tag of "TagsForFadeOut" is assigned to them
        /// </summary>
        ...
        /// </summary>
        public List<string> TagsForFading = new() { "FadeOut" };
        ...
        /// </summary>
        public List<string> TagsCausingLookUp = new() { "Terrain" };
        ...
        /// <summary>
        ///     If set to true, the character starts to fade when the camera's distance to its pivot is smaller than the "Character
        ///     Fade Start Distance"
        /// </summary>
        ...
        /// </summary>
        public float CharacterFadeOutAlpha;
        ...
        /// </summary>
        protected Dictionary<int, IEnumerator> _fadeInCoroutines = new();

        /// <summary>
        ///     Contains all currently active fade out coroutines
        /// </summary>
        protected Dictionary<int, IEnumerator> _fadeOutCoroutines = new();
        ...
        /// </summary>
        protected SortedDictionary<int, GameObject> _previousObjectsToFade = new();
            ...
            if (FadeObjectsBy == ObjectTriggerOption.Layer)
                for (var i = 0; i < 32; i++)
                    if (Utils.LayerInLayerMask(i, LayersForFading))
                        if (!Utils.LayerInLayerMask(i,
                                OccludingLayers)) // Layer for fading is not part of the occluding layers => throw a warning
                            Debug.LogWarning("Layer \"" + LayerMask.LayerToName(i) +
                                             "\" is set up for fading but not part of the occluding layers! Consider adding it when you want it to fade");

            if (LookUpTrigger == ObjectTriggerOption.Layer)
                for (var i = 0; i < 32; i++)
                    if (Utils.LayerInLayerMask(i, LayersCausingLookUp))
                        if (!Utils.LayerInLayerMask(i,
                                OccludingLayers)) // Layer for camera look up is not part of the occluding layers => throw a warning
                            Debug.LogWarning("Layer \"" + LayerMask.LayerToName(i) +
                                             "\" is set up for causing the camera look up but is not part of the occluding layers! Consider adding it");
        }
        ...
        protected virtual void Initialize(Vector3 from, Vector3 to, Camera cameraToUse)
        {
            ...
            var halfFieldOfView = cameraToUse.fieldOfView * 0.5f * Mathf.Deg2Rad;
            ...
            var targetDirection = from - to;
            targetDirection.Normalize();

            var localRight = cameraToUse.transform.right;
            var localUp = Vector3.Cross(-targetDirection, localRight);
            localUp.Normalize();

            var offset = targetDirection * cameraToUse.nearClipPlane;
        ...
        /// <summary>
        ///     Checks for objects inside the view frustum and - depending on the handling - fades them out or lets the camera zoom
        ///     in.
        ///     Returns -1 if there is no ambient occlusion, otherwise returns the closest possible distance so that the sight to
        ///     the target is clear
        /// </summary>
        /// <param name="from">Beginning of the view frustum</param>
        /// <param name="to">End of the view frustum</param>
        ...
        public virtual float CheckForOcclusion(Vector3 from, Vector3 to, Camera cameraToUse)
        {
            ...
            if (from == to) // No occlusion for the empty distance
            ...
            if (distance <= cameraToUse.nearClipPlane) // Do not check the distance which is anyhow not rendered
                return closestDistance;
            ...
            if (Shape == FrustumShape.Pyramid)
            {
                ...
                var down = -(_shiftUpperLeft - _shiftLowerLeft);
                var right = _shiftUpperRight - _shiftUpperLeft;
                ...
                for (var j = 0; j <= maxIndex; j++)
                {
                    ...
                    hitArray = Physics.RaycastAll(from, rayDirection, rayDirection.magnitude, OccludingLayers,
                    ...
                    if (hitArray.Length > 0)
                    {
                        ...
                        for (var n = 0; n < hitArray.Length; n++)
                        {
                            ...
                            if (FadeObject(hit.transform.gameObject)) // Skip objects which should be faded out
                                continue;

                            // Project the distance from the frustum edge onto the camera direction
                            var projectedDistance = Vector3.Project(rayDirection.normalized * hit.distance, direction)
                                .magnitude;
            ...
            else
            {
                ...
                var hitArray = BoxCastAll(from, to + direction * cameraToUse.nearClipPlane);
                ...
                for (var n = 0; n < hitArray.Length; n++)
                {
                    ...
                    if (hit.point ==
                        Vector3.zero) // Most likely due to the note described on https://docs.unity3d.com/ScriptReference/Physics.BoxCastAll.html
        ...
        {
            if (EnableCharacterFading) // Let the character fade in/out
            ...
            var objectsToFade = new SortedDictionary<int, GameObject>();

            #region Check for objects

            var hitArray = BoxCastAll(from, to);
            ...
            for (var n = 0; n < hitArray.Length; n++)
            {
                ...
                if (!FadeObject(hit.transform.gameObject)) // Object should not be faded out => skip
                    continue;

                var hitObjectID = hit.transform.GetInstanceID();

                if (!objectsToFade
                        .ContainsKey(
                            hitObjectID)) // Hit object is tagged for fading out and not yet tracked => fade it 
            ...
            var fadeOut = new List<GameObject>();
            var fadeIn = new List<GameObject>();
            ...
            var i = _previousObjectsToFade.GetEnumerator();
            var j = objectsToFade.GetEnumerator();

            var iFinished = !i.MoveNext();
            var jFinished = !j.MoveNext();
            var aListFinished = iFinished || jFinished;

            while (!aListFinished)
            {
                var iKey = i.Current.Key;
                var jKey = j.Current.Key;
                ...
                {
                    if (i.Current.Value != null) fadeIn.Add(i.Current.Value);
                ...
                {
                    if (j.Current.Value != null) fadeOut.Add(j.Current.Value);
            ...
            foreach (var o in fadeOut)
            {
                var objectID = o.transform.GetInstanceID();
                // Create a new coroutine for fading out the object
                var coroutine = FadeObjectCoroutine(FadeOutAlpha, FadeOutDuration, o);

                // Check if there is a running fade in coroutine for this object
                if (_fadeInCoroutines.TryGetValue(objectID, out var runningCoroutine))
                {
            ...
            foreach (var o in fadeIn)
            {
                var objectID = o.transform.GetInstanceID();
                // Create a new coroutine for fading in the object
                var coroutine = FadeObjectCoroutine(FadeInAlpha, FadeInDuration, o);

                // Check if there is a running fade out coroutine for this object
                if (_fadeOutCoroutines.TryGetValue(objectID, out var runningCoroutine))
                {
                    // Stop the already running coroutine
                    StopCoroutine(runningCoroutine);
        ...
        private RaycastHit[] BoxCastAll(Vector3 from, Vector3 to)
        {
            ...
            var boxHalfExtents = new Vector3(_halfWidth, _halfHeight, _cameraToUse.nearClipPlane * 0.5f);
            ...
            var boxCenter = from - direction * boxHalfExtents.z;
            var boxOrientation = Quaternion.LookRotation(direction);
            var maxDistance = distance - _cameraToUse.nearClipPlane;
            ...
            // Cast the box
            return Physics.BoxCastAll(boxCenter, boxHalfExtents, direction, boxOrientation, maxDistance,
                OccludingLayers, QueryTriggerInteraction.Ignore);
        }
        ...
        protected virtual IEnumerator FadeObjectCoroutine(float target, float duration, GameObject o)
        {
            ...
            var objectRenderers = o.transform.GetComponentsInChildren<Renderer>();
            ...
            if (objectRenderers.Length > 0)
            {
                ...
                var currentVelocity = new float[objectRenderers.Length];
                    ...
                    for (var i = 0; i < objectRenderers.Length; i++)
                    {
                        ...
                        foreach (var m in mats)
                            if (m.HasProperty("_Color"))
                            {
                                ...
                                if (alpha == -1.0f) // Compute the alpha only once
                        ...
                        if (Utils.IsAlmostEqual(alpha, target,
                                0.01f)) // The current alpha is almost equal to the target alpha value to => stop fading
                            continueFading = false;
                    }
        ...
        protected virtual void CharacterFade(Camera cameraToUse)
        {
            ...
            var closestPointToCharacter = transform.position;
            if (collider) closestPointToCharacter = collider.ClosestPointOnBounds(cameraToUse.transform.position);

            // Get the actual distance between the used camera and the character
            var actualDistance = Vector3.Distance(closestPointToCharacter, cameraToUse.transform.position);

            // Compute the new alpha value depending on the fading start and end distance
            var t = Mathf.Clamp01((actualDistance - CharacterFadeEndDistance) /
                                  (CharacterFadeStartDistance - CharacterFadeEndDistance));

            // Go through all renderers found for the character
            foreach (var r in _characterRenderersToFade) // Go through all their materials
        ...
        /// <summary>
        ///     Updates the _characterRenderersToFade, has to be called when the character renderers changed and character fading
        ///     is on
        /// </summary>
        protected virtual void UpdateCharacterRenderersToFade()
        {
            var renderers = new List<Renderer>();
            var temp = GetComponentsInChildren<Renderer>();

            var addRenderer = true;
        ...
        {
            if (!EnableCameraLookUp) return false;

            return Physics.Raycast(cameraToUse.transform.position, Vector3.down, out var hitInfo,
                       1.0f + ViewportMargin.y, OccludingLayers, QueryTriggerInteraction.Ignore)
        ...
        public virtual void DrawFrustum(Vector3 from, Vector3 to, Camera cameraToUse, bool withCameraPlane = false)
        {
            ...
            var frustumPlaneColor = Color.gray;
            var cameraPlaneColor = Color.yellow;
            var frustumEdgeColor = Color.white;

            // Calculate the near frustum plane at the end position (e.g. desired camera position)
            var upperLeft = to + _shiftUpperLeft;
            var upperRight = to + _shiftUpperRight;
            var lowerLeft = to + _shiftLowerLeft;
            var lowerRight = to + _shiftLowerRight;
