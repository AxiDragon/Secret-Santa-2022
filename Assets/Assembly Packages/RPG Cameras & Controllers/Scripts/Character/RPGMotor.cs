...
namespace JohnStairs.RCC.Character
{
    public abstract class RPGMotor : MonoBehaviour
    {
        ...
        /// <summary>
        ///     The speed of character movement while solely strafing. A combined speed is computed when also running/walking
        ///     forward/backwards
        /// </summary>
        ...
        /// <summary>
        ///     If set to true, jumps while the character is in midair are allowed. Used in combination with variable
        ///     "AllowedMidairJumps"
        /// </summary>
        public bool EnableMidairJumps;
        ...
        /// <summary>
        ///     If set to true, the character starts to dive only when moving forward (or backwards). Otherwise, the character
        ///     remains in an upright stance
        /// </summary>
        ...
        /// <summary>
        ///     If set to true, the character passively rotates with rotating objects, e.g. rotating platforms, when standing on
        ///     them. Requires MoveWithMovingGround
        /// </summary>
        public bool RotateWithRotatingGround = true;

        /// <summary>
        ///     If set to true, the character’s jumping direction is affected by the ground object, i.e. performing a standing jump
        ///     on a moving object lets the character always land on the same spot of the object
        /// </summary>
        ...
        /// <summary>
        ///     Tolerance height used for the grounded check. The larger the value, the larger the distance to the ground which
        ///     sets _grounded to true. Useful for tweaking movement on debris
        /// </summary>
        public float GroundedTolerance = 0.16f;

        /// <summary>
        ///     A value representing the degree at which the character starts to fall. The default value is 6 to let the character
        ///     be grounded when walking down small hills
        /// </summary>
        ...
        /// </summary>
        protected bool _autorunningStarted;

        /// <summary>
        ///     False if autorunning was started in the current frame
        /// </summary>
        protected bool _autorunningStopped;
        ...
        /// </summary>
        protected int _midairJumpsCount;
        ...
        protected virtual void Start()
        {
            ...
            {
                Debug.LogWarning(
                    "RPGMotor variable \"Ignored Layers\" does not contain the character's layer! Negative side effects might occur (see Manual section 1.2.3)");

                if (IgnoredLayers == 0)
                {
                    IgnoredLayers = 1 << LayerMask.NameToLayer("Player");
                    Debug.LogWarning(
                        "RPGMotor variable \"Ignored Layers\" was set to \"Nothing\". Defaulted the layer mask to layer \"Player\"");
                }
            }
        ...
        {
            if (!_swimming && !_groundedLastFixedUpdate &&
                _grounded) // Ensure that the character's X and Z axes rotations are reset after swimming (and flying) to prevent tilted ground movement
                ResetXandZrotations();

            _groundedLastFixedUpdate = _grounded;

            if (EnableCollisionMovement) ApplyCollisionMovement();
        }

        /// <summary>
        ///     "OnControllerColliderHit is called when the controller hits a collider while performing a Move" - Unity
        ///     Documentation
        /// </summary>
        ...
        protected virtual void OnDrawGizmosSelected()
        {
            ...
            var yellow = new Color(1.0f, 1.0f, 0.0f, 0.35f);
            var red = new Color(1.0f, 0.0f, 0.0f, 0.35f);
            var blue = new Color(0.0f, 0.0f, 1.0f, 0.55f);
            ...
            Gizmos.DrawCube(
                ...
                Vector3.up * (SwimmingStartHeight - _characterController.height * 0.5f),
                new Vector3(0.7f, 0.01f, 0.7f));
        }
        ...
        public virtual void StartMotor()
        {
            ...
            if (swamLastFrame && !_swimming) // Character stopped swimming this frame
                forceGroundedCheck = true;

            if (_movementDirection.y < 0 ||
                forceGroundedCheck) // Check for grounded only if we are drawn towards the ground
                _grounded = Physics.CheckSphere(_colliderBottom.center + Vector3.down * GroundedTolerance,
                    _colliderBottom.radius, ~IgnoredLayers, QueryTriggerInteraction.Ignore);
            else // Upward movement, e.g. jumping => disable grounded check so that colliders from the side cannot interrupt the movement
                _grounded = false;

            _enable3dMovement = IsSwimming() || IsFlying();

            if (_animator.GetBool("Jump")) _animator.ResetTrigger("Jump");
        }
        ...
        public virtual void SetAnimatorParameters(Animator animator)
        {
            ...
            animator.SetBool("Sliding",
                _slidingBuffer <
                -SlidingTimeout); // buffer full due to many consecutive sliding events => fire animation
            animator.SetBool("Swimming", _swimming);

            if (_animator.GetBool("Jump")) animator.SetTrigger("Jump");
        }

        /// <summary>
        ///     Immediately translates the character. Should be called from within FixedUpdate so that the Character Controller
        ///     component does not
        ...
        {
            if (IsCombatLockActive()) // Do not rotate if the character is locked on target
                return;
        ...
        protected virtual void ApplyCollisionMovement()
        {
            ...
            var start = _colliderBottom.center;
            var end = start + Vector3.up * (_characterController.height - _characterController.radius);
            var colliders = Physics.OverlapCapsule(start, end, _characterController.radius, ~IgnoredLayers,
                QueryTriggerInteraction.Ignore);

            foreach (var c in colliders)
            {
                if (c is MeshCollider) // Mesh colliders do not support the "ClosestPoint" method => skip
                    continue;
                ...
                var closestPointDirection = closestPoint - transform.position;
                // Add delta movement needed to move the character collider out of collider c 
                moveDirection += closestPointDirection.normalized *
                                 (closestPointDirection.magnitude - _characterController.radius);
            }

            if (moveDirection != Vector3.zero) // Apply the combined delta movements
        ...
        protected virtual void ApplyGravity()
        {
            ...
            if (!_enable3dMovement) _movementDirection.y -= Gravity * Time.deltaTime;
        }
        ...
        protected virtual void ResetXandZrotations()
        {
            ...
            var delta = _colliderBottom.center + Vector3.down * (_colliderBottom.radius + _colliderBottomOffset) -
                        transform.position;
            Translate(delta);
        ...
        protected virtual void ApplySliding()
        {
            ...
            var hits = Physics.SphereCastAll(_colliderBottom.center, _colliderBottom.radius, Vector3.down,
                GroundedTolerance, ~IgnoredLayers, QueryTriggerInteraction.Ignore);

            foreach (var hit in hits)
            {
                var hitNormal = hit.normal;
                // Compute the slope in degrees
                var slope = Vector3.Angle(hitNormal, Vector3.up);
                // Compute the sliding direction
                var slidingDirection = new Vector3(hitNormal.x, -hitNormal.y, hitNormal.z);
        ...
        protected virtual void PreventSwimmingAboveSurface()
        {
            ...
            if (_movementDirection.y * Time.deltaTime + _swimmingStartHeight + 0.5f >
                _currentWaterHeight) //  Prevent that the character can swim above the water level => cap the movement in Y direction
        ...
        {
            return transform.TransformPoint(_characterController.center) +
                   (side == ColliderSide.Top ? Vector3.up : Vector3.down) * _characterController.height * 0.5f;
        }
        ...
        {
            _colliderBottom.center =
                GetColliderPosition(ColliderSide.Bottom) + Vector3.up * _characterController.radius;
        ...
        protected virtual void PerformJump()
        {
            ...
            {
                if (_inputDirection.x != 0 || _inputDirection.z != 0) _runningJump = true;
        ...
        {
            if (toggle && !_swimming) _crouching = !_crouching;
        }
        ...
        {
            if (toggle && !_swimming) _walking = !_walking;
        }
        ...
        {
            var result = _autorunningStarted;
        ...
        {
            var result = _autorunningStopped;
        ...
        protected virtual float ApplyMovementSpeedMultipliers(float current)
        {
            ...
            else
            {
                ...
                if (_sprinting) result *= SprintSpeedMultiplier;
                // Set the speed if walking is enabled
                if (_walking) result = Mathf.Min(WalkSpeed, result);
            }
        ...
        {
            if (!Time.inFixedTimeStep) _characterController.enabled = false;

            transform.position = targetPosition;
            UpdateColliderBottom();

            if (!Time.inFixedTimeStep) _characterController.enabled = true;
        }
        ...
        /// <param name="withYrotation">
        ///     If true, the character's Y axis rotation is additionally aligned with the Y axis rotation
