...
namespace JohnStairs.RCC
{
    public class MovingPlatform : MonoBehaviour
    {
        ...
        protected virtual void Start()
        {
            ...
            if (!_triggerCollider)
                Debug.LogWarning("No trigger collider on game object " + name +
                                 " found! Please attach a collider with \"Is Trigger\" = true to make the MovingPlatform component work");
        ...
        protected virtual void FixedUpdate()
        {
            ...
            var delta = transform.rotation * Quaternion.Inverse(lastRotation);

            // Move (and rotate) all passengers
            foreach (var entry in _localPassengerPositions)
            {
                var rpgMotor = entry.Key;
                rpgMotor.TeleportTo(transform.TransformPoint(entry.Value));

                if (rpgMotor.RotateWithRotatingGround) rpgMotor.Rotate(delta);
            }
        ...
        protected virtual void OnDrawGizmos()
        {
            ...
            if (StartingPoint) Gizmos.DrawWireCube(StartingPoint.position, transform.localScale);

            if (TurningPoint) Gizmos.DrawWireCube(TurningPoint.position, transform.localScale);
        }
        ...
        {
            var rpgMotor = other.GetComponent<RPGMotor>();

            if (rpgMotor?.MoveWithMovingGround ?? false) _passengers.Add(rpgMotor);
        }
        ...
        {
            var currentPassengers = new HashSet<RPGMotor>();
            ...
            foreach (var passenger in _passengers)
                if (!passenger.GroundAffectsJumping || IsAbovePlatform(passenger))
                {
                    ...
                    _localPassengerPositions.Add(passenger,
                        transform.InverseTransformPoint(passenger.transform.position));
                }
        ...
        /// <summary>
        ///     Checks if a passenger is still above the platform. Usually used for checking passengers with "GroundAffectsJumping"
        ///     to see if
        ...
        {
            var triggerSize = _triggerCollider.size; // local trigger collider size
            var characterPositionLocal = transform.InverseTransformPoint(rpgMotor.transform.position);

            if (characterPositionLocal.y <
                _triggerCollider.center.y -
                triggerSize.y * 0.5f) // Position is below the box collider => passenger left the platform
                return false;

            // RPGMotor has the Character Controller as a prerequisite => get the radius of its collider
            var characterControllerRadius = rpgMotor.GetComponent<CharacterController>().radius;
            ...
            if (characterPositionLocal.x - characterControllerRadius <= triggerSize.x * 0.5f
                ...
                && characterPositionLocal.z + characterControllerRadius >=
                -triggerSize.z * 0.5f) // Passenger is between the bounds of the trigger collider in the XZ plane
                return true;
